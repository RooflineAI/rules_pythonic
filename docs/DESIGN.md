...
- **Remote execution: install locally, test remotely.** `PythonicInstall` and `PythonicWheel` are tagged `no-remote-exec`, which forces them to run on the local machine where `UV_CACHE_DIR`, `sandbox_writable_path`, and hardlinks all work. The resulting TreeArtifacts are still uploaded to the remote cache. Downstream actions (tests, binaries) run on remote workers and download the cached packages directory via runfiles. This means the install action never needs to work on a remote worker — it benefits from the local uv cache, and remote workers only see the finished output.
